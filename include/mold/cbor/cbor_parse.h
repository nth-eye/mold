...
{
    // Strip the field_t wrapper BEFORE the optional wrapper: a member
    // declared as field_t<K, std::optional<inner_t>> must count inner_t's
    // members, and decay_optional_t of the field wrapper is a no-op.
    ...
    if constexpr (has_spec_read<U>) {
        ...
        constexpr size_t nested = [] <size_t... I> (std::index_sequence<I...>) {
            size_t m = 0;
            ((m = std::max(m, cbor_count_nodes<std::tuple_element_t<I, typename type_info_t<U>::tuple_type>>())), ...);
            return m;
